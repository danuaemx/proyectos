...
def ordenar(p1,p2):
    global A
    if A[p1-1]=='Infinito' and  A[p2-1]=='Infinito':
        pass
    if A[p1-1]!='Infinito':
        pass
    if  A[p2-1]!='Infinito':
        A[p1-1]= A[p2-1]
    if A[p1-1]!='Infinito' and  A[p2-1]!='Infinito' and A[p2-1] >= A[p1-1]:
        A[p2-1]='Infinito'
    else:
        A[p1-1]=A[p2-1]
        A[p2-1]='Infinito'
...
def main():
    ...
    for i in range(1, n + 1):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=k0) as executor:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=n2) as executor:
            executor.map(lambda pos: cmp(pos), P)
    print(A)
    if p == 'No encontrado':
        print(p)
    else:
